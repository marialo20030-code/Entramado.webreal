...
export function UploadModal({ isOpen, onClose, folders, onSuccess, postToEdit }: UploadModalProps) {
  ...
  const [imagePreviews, setImagePreviews] = useState<string[]>(postToEdit?.images || []);
  ...
  useEffect(() => {
    ...
    if (postToEdit) {
      ...
      // Si no hay post para editar, intentar cargar desde localStorage
      ...
      if (savedDraft) {
        ...
        setCustomThumbnail(savedDraft.customThumbnail || '');
      ...
      <div className="bg-[#f5f1e8] border-b border-gray-300 flex items-center justify-between px-4 py-2 flex-shrink-0 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            className="px-3 py-1.5 text-sm bg-[#fefcf8] border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-400 outline-none transition-all shadow-sm"
          >
                ...
                {mediaType === 'image' && (
                  <div className="space-y-3">
                    ...
                    >
                      ...
                      <span className="text-gray-700">Agregar imágenes</span>
                    </button>
                  </div>
                )}
