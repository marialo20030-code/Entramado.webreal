...
import { Plus, Grid3x3, Calendar, FolderOpen, LogOut, FileText, X, Search, User } from 'lucide-react';
    ...
    posts.forEach(async (post) => {
      if (!postColors[post.id]) {
        try {
          ...
          setPostColors((prev) => ({ ...prev, [post.id]: colors }));
        } catch (err) {
    ...
    <DynamicBackground colors={postColors}>
      <nav className="bg-white/80 backdrop-blur-sm border-b border-gray-100 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between gap-4">
            <h1 className="text-2xl font-light text-gray-800">Inspiración</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        {loading ? (
          ...
          <CalendarView posts={filteredPosts} onPostClick={(post) => setSelectedPost(post)} />
        ) : view === 'profile' && user ? (
          <ProfileView
            userId={user.id}
